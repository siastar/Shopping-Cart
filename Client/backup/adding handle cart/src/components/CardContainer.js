...
class CardContainer extends React.Component {
    ...
    onCartModify = (buttonType) => {
        console.log('modifying cart', buttonType);
    }
    ...
    handleCartButton = () => {
        console.log('add button clicked (parent message)');
        ...
        if ('qt' in currentProduct === false)  {
            currentProduct.qt = 1;
            console.log('added qt property to current product')
        }
        
        console.log('current product already has qt property_' , currentProduct.qt);
        ...
        console.log('temp cart is: ',temporaryCart);
        ...
        if(temporaryCart.length > 1) { //that it means if the array has at least 2 objects            
            ...
            console.log('scanning duplicates: last added product: \n' , lastIn);
            for (let i = temporaryCart.length - 2 ; i >= 0 ; i-- ) {
                //for any element in the array excluded the last added one
                console.log('scanning duplicates: array index: ', i , ' ---  obj id: ' , temporaryCart[i].id);
                if (lastIn.id === temporaryCart[i].id){
                    console.log('scanning duplicates: found a duplicate with id: ', lastIn.id);
                    temporaryCart[i].qt ++;
                    temporaryCart.pop();
                    console.log('added already existing item to temp cart: \n', temporaryCart)
                }        
            }          
        }
        else{
        ...
        console.log('temp cart will update the state: \n', temporaryCart)
        this.setState({cart: temporaryCart});
        console.log('last state: ' , this.state);
        console.log('last state cart:' , this.state.cart);        
    }
